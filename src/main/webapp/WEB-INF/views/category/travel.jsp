  ...
  <section class="breadcrumbs">
	...
	<div class="container">
		...
		<div class="pagination">
			<!-- 서울 지역 페이지 네이션 구현 / 지역벌 페이지는 ajax로 구현함-->
				...
				<ul class="pagination">
						...
						<c:choose>
							<c:when test="${curPage - 5 < 1}">
								<li class="page-item"><a class="page-link"
									href="/category/travel?contentTypeId=${recommendTour[0].contenttypeid}&page=1">이전</a></li>
							</c:when>
							<c:otherwise>
								<li class="page-item"><a class="page-link"
									href="/category/travel?contentTypeId=${recommendTour[0].contenttypeid}&page=${curPage - 5}">이전</a></li>
							</c:otherwise>
						</c:choose>
					</li>
					...
					<c:forEach var="pageNum" begin="${startPage}" end="${endPage}">
						<li class="page-item ${pageNum == curPage ? 'active' : ''}">
							<a class="page-link"
							href="/category/travel?contentTypeId=${recommendTour[0].contenttypeid}&page=${pageNum}">${pageNum}</a>
						</li>
					</c:forEach>
					<c:choose>
						<c:when test="${curPage + 5 >= totalPages}">
							<li class="page-item"><a class="page-link"
								href="/category/travel?contentTypeId=${recommendTour[0].contenttypeid}&page=${totalPages}">다음</a></li>
						</c:when>
						<c:otherwise>
							<li class="page-item"><a class="page-link"
								href="/category/travel?contentTypeId=${recommendTour[0].contenttypeid}&page=${curPage + 5}">다음</a></li>
						</c:otherwise>
					</c:choose>
					<li class="page-item">
