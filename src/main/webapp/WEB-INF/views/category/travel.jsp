...
<main id="main">
 ...
 <div class="container">
 		<div class="mt-3">
		    <label for="search" class="form-label">키워드 입력하세요.</label>
		    <input style="width: 20%; height: 40px;" id="search" value="">
	...
	<div class="pagination">      <!-- 서울 지역 페이지 네이션 구현 / 지역벌 페이지는 ajax로 구현함-->
		<nav aria-label="Page navigation example">
		  <ul class="pagination">
		    <li class="page-item">
    		    <a class="page-link" href="/category/travel?contentTypeId=${recommendTour[1].contenttypeid}&page=1" aria-label="Previous">
			      <span aria-hidden="true">&laquo;</span>
		        </a>
		    <c:choose>
		    	<c:when test="${curPage - 5 < 1}">
			      <li class="page-item"><a class="page-link" href="/category/travel?contentTypeId=${recommendTour[1].contenttypeid}&page=1">이전</a></li>
		      	</c:when>
		      	<c:otherwise>
		      		<li class="page-item"><a class="page-link" href="/category/travel?contentTypeId=${recommendTour[1].contenttypeid}&page=${curPage - 5}">이전</a></li>
		      	</c:otherwise>
	      	</c:choose>
		    </li>
		    ...
		    <c:forEach var="pageNum" begin="${startPage}" end="${endPage}">
				<li class="page-item ${pageNum == curPage ? 'active' : ''}">
					<a class="page-link" href="/category/travel?contentTypeId=${recommendTour[1].contenttypeid}&page=${pageNum}">${pageNum}</a>
				</li>
			</c:forEach>
			<c:choose>
				<c:when test="${curPage + 5 >= totalPage}">
					<li class="page-item"><a class="page-link" href="/category/travel?contentTypeId=${recommendTour[1].contenttypeid}&page=${totalPages}">다음</a></li>
			    </c:when>
			    <c:otherwise>
			    	<li class="page-item"><a class="page-link" href="/category/travel?contentTypeId=${recommendTour[1].contenttypeid}&page=${curPage + 5}">다음</a></li>
			    </c:otherwise>
	    	</c:choose>
    	   	 <li class="page-item">
		      <a class="page-link" href="/category/travel?contentTypeId=${recommendTour[1].contenttypeid}&page=${totalPages}" aria-label="Next">
